...
class ToveTestCases {
	...
	@Test
	void test() {
		TFloatArrayList tab = new TFloatArrayList(100);
		for (int i = 0; i < 100; i++) {
			tab.add(i*1.0f);
		}
		tab.toArray();
        ...
        cuMemAlloc(deviceInput, tab.size() * Sizeof.FLOAT);
        long timecp0 = System.nanoTime();
        cuMemcpyHtoD(deviceInput, Pointer.to(tab.toArray()), 
        		tab.size() * Sizeof.FLOAT);
        long timecp1 = System.nanoTime();
        long durationcopy = timecp1 - timecp0;
        long time0 = System.nanoTime();
        float resultJCuda = helloworld(deviceInput, tab.size());
